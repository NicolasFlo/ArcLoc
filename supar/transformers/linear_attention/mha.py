...
class RMSNorm(nn.Module):
    "Construct a RMSnorm module (See citation for details)."

    def __init__(self, features, eps=1e-6):
        super(RMSNorm, self).__init__()
        self.a_2 = nn.Parameter(torch.ones(features))
        self.b_2 = nn.Parameter(torch.zeros(features))
        self.coeff = features ** (-1. / 2)
        self.eps = eps

    def forward(self, x):
        norm = torch.norm(x, p=2, dim=-1, keepdim=True)
        rms = self.coeff * norm
        return self.a_2 * x / (rms + self.eps) + self.b_2
...
class MHA(nn.Module):
    ...
    def __init__(self, d_model, n_heads, use_cos,
                 layer_norm_choice, kernel,
        ...
        Args:
          ...
          layer_norm_choice: (str) Choice of layer normalization to be used by the 
            linear transformer or cosformer. 'layer', 'RMS', 'None'.
        ...
        if not use_cos and layer_norm_choice in ['layer', 'RMS']:
        ...
        elif layer_norm_choice in ['layer', 'RMS']:
            self.attention_func = self.norm_attention
        else:
            self.attention_func = self.linear_attention
        if layer_norm_choice == 'layer':
            self.ln = nn.LayerNorm(d_model)
        elif layer_norm_choice == 'RMS':
    ...
    def norm_attention(self, q, k, v, span_mask, weights=None):
        """Implements normattention"""

        # q, k, v -> [batch_size, seq_len, n_heads, d_head]

        kv = torch.einsum('bsnx,bsnz->bnxz', k, v)
        # kv -> [batch_size, n_heads, d_head, d_head]
        batch_size, n_heads, d_head, _ = kv.size()

        output = torch.einsum('bsnx,bnxz->bsnz', q,
                              kv).contiguous()
        # output -> [batch_size, seq_len, n_heads, d_head]

        return output, None, None

    def linear_attention(self, q, k, v, span_mask, weights=None):
        ...
        if span_mask is None:
            ...
            k_masked = torch.einsum('bsnx,rs->brnx', k, span_mask)
            kv = torch.einsum('bsnx,bsnz->bnxz', k_masked, v)
        # kv -> [batch_size, n_heads, d_head, d_head]
        batch_size, n_heads, d_head, _ = kv.size()
    ...
    def cos_linear_attention(self, q, k, v, span_mask, weights):
        ...
        Args:
          ...
          weights: (tuple of (torch.Tensor of float32)[batch_size, seq_len])
            weights = (cos, sin), where cos[i, j] = cos(pi * i / 2 / M[i]) where M[i] is the length
    ...
    def forward(self, x, mask, weights, span_mask, use_causal_mask):
        ...
        Args:
          ...
          weights: (tuple of (torch.Tensor of float32)[batch_size, seq_len, n_heads, d_head])
            weights = (cos, sin), where cos[i, j] = cos(pi * i / 2 / M[i]) where M[i]
        ...
        k = self.apply_mask(self.kernel(k), mask)
        
        if use_causal_mask:
            seq_len_root = int(k.size(1)**(1/2))
            seq_mask = torch.zeros_like(k)
            for h in range(seq_mask.size(0)):
                for i in range(seq_len_root):
                    for j in range(seq_len_root):
                        if i > j:
                            seq_mask[h][i*seq_len_root + j][:] = 1
            seq_mask = torch.where(seq_mask == 0, False, True)
            k = k.masked_fill(seq_mask, 0)
        k = self.split_heads(k)

        v = self.split_heads(v)
        # q, k, v -> [batch_size, seq_len, n_heads, d_head]

        x = self.attention_func(q, k, v, span_mask, weights)
        # x -> [batch_size, seq_len, n_heads, d_head]
