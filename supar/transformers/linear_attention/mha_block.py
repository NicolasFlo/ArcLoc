...
class MHA_block(nn.Module):
    ...
    def __init__(self, d_model, n_heads, use_cos, layer_norm_choice,
                 kernel, dropout,
        ...
        self.mha = MHA(
            d_model, n_heads, use_cos, layer_norm_choice,
            kernel, dropout, denom_eps, bias)
    ...
    def forward(self, x, mask, weights, span_mask, use_causal_mask):
        ...
        fx, sim_cos, sim_sin = self.mha(self.ln1(x), mask, weights, span_mask, use_causal_mask)
        x = x + fx
...
class MHA_block_rezero(nn.Module):
    ...
    def forward(self, x, mask, weights, span_mask):
        """Implements forward pass.
        ...
        """
        # x -> [batch_size, seq_len, d_model]
        fx, sim_cos, sim_sin = self.alpha * self.mha(x, mask, weights, span_mask)
        x = x + fx
