...
class Kernel_transformer(nn.Module):
    ...
    def __init__(self, use_cos, kernel, 
                 d_model, n_heads, n_layers, n_emb, layer_norm_choice='layer',
                 ffn_ratio=4, rezero=True, ln_eps=1e-5, denom_eps=1e-5,
        ...
        Args:
          ...
          layer_norm_choice: (str) Choice of layer normalization to be used by the linear transformer or cosformer.
            'layer', 'RMS', 'None'.
          ...
          use_causal_mask: (bool) Whether or not to use a causal mask in the sequence.
        """
        ...
        self.layer_norm_choice = layer_norm_choice
    ...
    def get_cos_weights(self, lengths, max_len=None):
        ...
        Returns:
          (cos, sin): (tuple of torch.Tensor of float32)[batch_size, seq_len]).
            cos[i, j] = cos(pi * i / 2 / M[i]) where M[i] is the length
    ...
    def forward(self, input, attention_mask=None, lengths=None, span_mask=None):
        ...
        Args:
          ...
          attention_mask: torch.Tensor of long or bool [batch_size, seq_len]. Has 0 for
            ...
            Compatible with Huggingface pipeline. If None is given, masking will be ignored.
        ...
        x = input  # self.emb_in(input_ids)
        # if not self.use_cos:
        #     x += self.emb_pos(x)

        for block in self.mha_blocks:
            x = block(x, attention_mask, cos_weights, span_mask, self.use_causal_mask)
