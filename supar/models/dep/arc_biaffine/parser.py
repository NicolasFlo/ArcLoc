...
import torch
from supar.models.dep.arc_biaffine.model import ArcBiaffineDependencyModel
...
class ArcBiaffineDependencyParser(Parser):
    r"""
    ...
    NAME = 'arc-biaffine-dependency'
    MODEL = ArcBiaffineDependencyModel
    ...
    def train_step(self, batch: Batch) -> torch.Tensor:
        ...
        mask[:, 0] = 0
        s_arc, s_rel, s_arc_filter, selected_arcs = self.model(words, feats, arcs)
        loss, filter_loss, oracle_score = self.model.loss(s_arc, s_rel, arcs, rels, mask, s_arc_filter, selected_arcs, self.args.partial)
        return loss, filter_loss, oracle_score
    ...
    def eval_step(self, batch: Batch) -> AttachmentMetric:
        ...
        # ignore the first token of each sentence
        mask[:, 0] = 0
        if self.args.average_epochs and self.epoch >= self.args.start_averaging:
            s_arc, s_rel, s_arc_filter, selected_arcs = self.avg_model(words, feats)
        else:
            s_arc, s_rel, s_arc_filter, selected_arcs = self.model(words, feats)
        loss, filter_loss, oracle_score = self.model.loss(s_arc, s_rel, arcs, rels, mask, s_arc_filter, selected_arcs, self.args.partial)
        arc_preds, rel_preds = self.model.decode(s_arc, s_rel, mask, self.args.tree, self.args.proj, selected_arcs)
        if self.args.partial:
            mask &= arcs.ge(0)
        # ignore all punctuation if not specified
        if not self.args.punct:
            mask.masked_scatter_(mask, ~mask.new_tensor([ispunct(w) for s in batch.sentences for w in s.words]))
        return AttachmentMetric(loss, (arc_preds, rel_preds), (arcs, rels), mask), filter_loss, oracle_score
    ...
    def pred_step(self, batch: Batch) -> Batch:
        ...
        mask[:, 0] = 0
        if self.args.average_epochs and self.epoch >= self.args.start_averaging:
            s_arc, s_rel, s_arc_filter, selected_arcs = self.avg_model(words, feats, None)
        else:
            s_arc, s_rel, s_arc_filter, selected_arcs = self.model(words, feats, None)
        arc_preds, rel_preds = self.model.decode(s_arc, s_rel, mask, self.args.tree, self.args.proj, selected_arcs)
